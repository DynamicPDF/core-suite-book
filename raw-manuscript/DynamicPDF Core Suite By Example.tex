...
\documentclass[american]{tufte-book}
\usepackage{amsmath}
\usepackage{amsthm}
...
\includeonly{chapter15}

\title[DynamicPDF Core Suite for .NET by Example]{%
\noindent\hspace*{-.25in}\parbox{1.25\linewidth}{%
\textcolor{white}{\fontsize{28}{30}\selectfont\bfseries
\textls[-30]{DynamicPDF Core Suite for .NET by Example}}%
\par\vspace{-0.25em}
}}
...
\author{%
  {\color{white}\bfseries
   \makebox[\linewidth][r]{\hspace*{3.00in}James A Brannan}%
  }%
}
...
\geometry{
  paperwidth=7in,
  paperheight=10in,
  bindingoffset=0.30in
}

\setlength{\textwidth}{256.94pt}
\setlength{\marginparwidth}{118.59pt}
\setlength{\marginparsep}{19.76pt}

%%%%% widen code listings

\usepackage{etoolbox}

\BeforeBeginEnvironment{lstlisting}{\begin{fullwidth}}
\AfterEndEnvironment{lstlisting}{\end{fullwidth}}
...
\renewenvironment{theindex}
  {%
    ...
    \footnotesize
    \begin{adjustwidth*}{0in}{-0.35in} % widen index into margins
    \setlength{\columnsep}{35pt}           % widen space between columns
    \begin{multicols}{2}
    \raggedright
...
\noindent
This work is openly licensed via CC BY 4.0.\par
\vspace{6pt}
\noindent\includegraphics[scale=.5]{figures/cc-by.png}
\noindent\begin{flushleft}
You are free to:
\begin{itemize}
...
\chapter*{}
\begin{center}
For Alan L. Winick, Esquire. Thanks for everything.
\newline-James
\par\end{center}
...
Part III, OtherTopics, covers additional topics, including custom components, report generation, and using DynamicPDF Designer for advanced document layout and automation.
...
\begin{figure}
\includegraphics[scale=0.40]{figures/ch1/fig2}\caption{NuGet packages used by book's project.}\label{mar:Nuget-packages-used}
\end{figure}
...
\includegraphics[scale=0.575]{figures/ch1/create-steps}\caption{Common processing steps in creating a PDF document.}\label{mar:Common-processing-steps-1}
\end{marginfigure}
...
\begin{lstlisting}[caption={Creating a PDF from scratch.},label={lis:Creating-a-PDF},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{figure}
\includegraphics[scale=0.25]{figures/ch1/fig4}\caption{A simple PDF document created by Listing \ref{lis:Creating-a-PDF}.}\label{fig:A-simple-PDF}
\end{figure}
...
you create an instance of \texttt{Label}\index{ceTe.DynamicPDF.PageElements.Label}
class and add it to our page. Classes inheriting from the\texttt{
PageElements} parent class include the following name-spaces:\marginnote{Modifying a PDF almost always requires performing the steps in Figure  \ref{mar:mod-common-processing-steps-1}} 
\begin{itemize}
...
Now that you have seen how to create your first PDF document, let's switch to modifying three pre-existing PDF documents by merging them together. Let's also add a new page to the merged document. 
\begin{figure}
...
\begin{figure}
\includegraphics[scale=0.30]{figures/ch1/fig7}\caption{Merged PDF document.}\label{mar:merged-pdf}
\end{figure}
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Modifying a PDF.},label={lis:modifying-a-PDF},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
...
\begin{marginfigure}
\includegraphics[scale=0.26]{figures/ch2/error}\caption{Attempting to output a document with no pages.}\label{mar:Attempting-to-output}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Creating a simple PDF Document.},label={lis:ch2Creating-a-simple},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
...
\begin{marginfigure}
\includegraphics[scale=0.28]{figures/ch2/fig2}\caption{Listing \ref{lis:ch2-multipage-Creating-a-PDF-1} produces a five-page
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Creating a PDF with different page sizes.},label={lis:ch2-multipage-Creating-a-PDF-1},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
...
\begin{figure}
\includegraphics[scale=0.38]{figures/ch2/coords}\caption{Coordinates of an $8.5x11$ page with 50 point margins.}\label{fig:ch2-Coordinates-of-an}
\end{figure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding three circles to a page.},label={lis:Adding-two-circles},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
PDF document. 
\begin{figure}
\includegraphics[scale=0.56]{figures/ch2/ignore-marg}\caption{Ignoring an element's margins and relative positioning example.}\label{fig:Ignoring-an-element's}

\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Ignoring an image's margins and relative positioning.},label={lis:Ignoring-an-image's},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
...
at that point.\begin{marginfigure}
\includegraphics[scale=0.26]{figures/ch2/stepone-center}\caption{Step one of centering the image in Listing \ref{lis:Centering-an-image}.}\label{mar:Step-one-of}
\end{marginfigure} Remember, an image's \texttt{x} and \texttt{y} coordinates are the top
left corner of the image. 
\begin{figure}
\includegraphics[scale=0.40]{figures/ch2/layoutgrid1}\caption{The image from Listing \ref{lis:Centering-an-image} centered on a
PDF document page.}\label{mar:center-The-image-from}
\end{figure}
For example, the image used in Listing \ref{lis:Centering-an-image}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Centering an image on a page.},label={lis:Centering-an-image},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
...
\begin{figure}
\includegraphics[scale=0.45]{figures/ch2/fig11}\caption{Centering an image.}\label{mar:centering-image}
\end{figure}
\begin{figure}
\includegraphics[scale=0.26]{figures/ch2/image-coords-one}\caption{Image centered relative to margin compared to relative to page.}\label{mar:Image-marg-relative}
\end{figure}
...
to the page (Figure \ref{mar:mar:Image-marg-relative}). 
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Modifying Listing \ref{lis:Centering-an-image} to different image
positions.},label={lis:Modifying-Listing-},basicstyle={\footnotesize\ttfamily},breaklines=true]
Document myDoc = new();
Page myPg = new();
myDoc.Pages.Add(myPg);
...
\begin{figure}
\includegraphics[scale=0.49]{figures/ch2/valign}\caption{Applying VAlign to a Label.}\label{mar:Applying-VAlign-to}
\end{figure}
...
\begin{figure}
\includegraphics[scale=0.30]{figures/ch2/grid-example}\caption{Using the LayoutGrid class for development.}\label{fig:layout-gridExample-using-the}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Using the LayoutGrid class for development.},label={lis:ly-grid-Example-using-the},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={HTMl document with simple CSS and image link.},label={lis:HTMl-document-with},basicstyle={\footnotesize\ttfamily},breaklines=true]
<html>
<head>
    <style>
        p {
...
image available on the DynamicPDF website.\marginnote{The \texttt{TextGenerator} class is a utility class in the GitHub
project that reads the HTML into a string from a file.} Listing \ref{lis:Using-the-HtmlLayout}
loads the HTML as text and a \texttt{PageInfo}\index{PageInfo} instance to create a new \texttt{HtmlLayout}\index{HtmlLayout}
instance. You then add a header and footer and then create the PDF document (Figure \ref{fig:Using-an-HtmlLayout}).
\begin{figure}
...
\end{figure}
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Using the HtmlLayout class to create a PDF document.},label={lis:Using-the-HtmlLayout},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
...
to the document.\begin{marginfigure}
\includegraphics[scale=0.31]{figures/ch2/fig1}\caption{Document properties dialog.}\label{fig:Document-properties-dialog.}
\end{marginfigure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Document level properties.},label={lis:ch2-doc-prop-Creating-a-document},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
...
PDF document (Figure \ref{mar:A-custom-property}). 
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Setting viewer preferences.},label={lis:Setting-viewer-preferences},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
namespace ChapterExamples.Chapters.ChapterTwo
{
...
the \texttt{Page} class.\marginnote{Remember, always try to use relative measurements rather than hard
values.} Next you learned how to size a page and create
...
abstracts these page sizes.\marginnote{Do not forget to consider the margin in your layout calculations.}

After building a document with pages, we then discussed the coordinate
system and how to properly layout document pages. As you saw, a letter ($8.5\times11$) page is 612 points wide and
...
and \texttt{HeaderFooterTemplate}.\marginnote{The listing in this chapter refer to the code DocumentExampleGenerator.Generate(doc);.
...
GitHub project.}  An \texttt{EvenOddTemplate} adds
...
its elements above a page's content.
...
a new \texttt{Template}\index{Template} instance.\begin{marginfigure}
\includegraphics[scale=0.23]{figures/ch3/fig7}\caption{Templates adds consistent formatting.}\label{fig:template-formatting}
\end{marginfigure} You then add two rectangles, an image,
...
\begin{figure}
\includegraphics[scale=0.23]{figures/ch3/fig1}\caption{Applying a template to a five page PDF document.}\label{fig:tmpl-Applying-a-template}
\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding a template to a PDF document.},label={lis:Adding-a-template},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
using ChapterExamples.Utility;
...
elements.
\begin{marginfigure}
\includegraphics[scale=0.26]{figures/ch3/fig8}\caption{Stamp Template places elements in foreground.}\label{fig:template-formatting}
\end{marginfigure}
...
\begin{figure}
\includegraphics[scale=0.32]{figures/ch3/fig2}\caption{Applying a stamp template to a five page PDF document.}\label{fig:Applying-a-stamp}
\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Using a stamp template.},label={lis:forground-Using-a-stamp},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
using ChapterExamples.Utility;
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Applying an EvenOddTemplate to a PDF document.},label={lis:Applying-diffent-template},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
...
\includegraphics[scale=0.33]{figures/ch3/fig3}\caption{Applying an even/odd template to a PDF document.}\label{fig:Applying-an-even/odd}
\end{figure}
...
\begin{figure}
\includegraphics[scale=0.42]{figures/ch3/fig6}\caption{Applying a header and footer to a PDF document.}\label{fig:Applying-a-header}
\end{figure}
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Using the HeaderFooterTemplate class.},label={lis:Applying-headers-and},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
using ChapterExamples.Utility;
...
\marginnote{A section's page numbering is for the PDF Reader's benefit, not for
...
\end{marginfigure}
In Listing\ref{lis:Applying-sectioning-to}, you create a document with three
...
(Figure \ref{fig:final-Example-of-applying}).

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Applying sectioning to a PDF document.},label={lis:Applying-sectioning-to},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
...
\includegraphics[scale=0.31]{figures/ch3/fig4}\caption{Applying a header/footer template to a PDF document.}\label{fig:Applying-a-header/footer}
\end{figure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Omitting template from a single page of a document.},label={lis:Omitting-template-from},basicstyle={\footnotesize\ttfamily},breaklines=true]
public static void Create(string outputPath)
{
  Document doc = new Document();
  Template tmp = new Template();
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Omitting template from a section page.},label={lis:Omitting-template-from-1},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={A PDF document with multiple sections.},label={lis:A-PDF-document},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
using ChapterExamples.Utility;
...
\includegraphics[scale=0.35]{figures/ch3/fig5}\caption{Applying templates to a sectioned PDF document.}\label{fig:final-Example-of-applying}
...
the \texttt{ceTe.DynamicPDF\allowbreak.PageElements}\index{ceTe.DynamicPDF.PageElements} namespace while the base
...
\begin{itemize}
\item \texttt{ceTe.DynamicPDF.PageElements.Forms}\index{ceTe.DynamicPDF.PageElements.Forms}
\item \texttt{ceTe.DynamicPDF.PageElements.BarCoding\index{ceTe.DynamicPDF.PageElements.BarCoding}}
\end{itemize}
...
mistakenly use the \texttt{ceTe.DynamicPDF\allowbreak.LayoutEngine\allowbreak.LayoutElements}
...
\begin{marginfigure}
\includegraphics[scale=0.43]{figures/ch4/fig15}\caption{Visual Studio and conflicting namespace.}\label{mar:Visual-Studio-and}
...
\includegraphics[scale=0.60]{figures/ch4/fig14}\caption{Do not use LayoutElements namespace.}\label{fig:namespace-Do-not-use}
...
\begin{marginfigure}
\raggedright
\includegraphics[scale=0.20]{figures/ch4/fig1}\caption{Adding a Label element to a PDF document.}\label{fig:label-Result-from-adding}
\end{marginfigure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding a label to a PDF document's page.},label={lis:Adding-a-label},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
...
\begin{marginfigure}
\includegraphics[scale=0.18]{figures/ch4/fig2}\caption{Adding two TextArea elements to a document.}\label{mar:Results-from-adding}
\end{marginfigure}
...
the resulting PDF.
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding a TextArea to a PDF document.},label={lis:Adding-a-TextArea},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
using ChapterExamples.Utility;
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Handling text overflow.},label={lis:Handling-text-overflow},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
using ChapterExamples.Utility;
...
which we cover next.
\begin{figure}
\includegraphics[scale=0.36]{figures/ch4/fig4}\caption{A PDF document with a formatted text area.}\label{mar:Results-from-adding-1}
\end{figure}
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding a FormattedTextArea to a PDF document.},label={lis:Adding-a-FormattedTextArea},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding an HtmlArea.},label={lis:Adding-an-HtmlArea},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ChapterExamples.Utility;
...
or you can specify your own custom type \bgroup\inputencoding{latin9}\lstinline[basicstyle={\footnotesize\ttfamily},breaklines=true]!public UnorderedListStyle(string type, Font font)!\egroup
.
...
instance.\begin{marginfigure}
\includegraphics[scale=0.43]{figures/ch4/fig7}\caption{Results from adding an unordered list to a PDF document.}\label{mar:Results-from-adding-2}
\end{marginfigure} The sub-list, as it is also unordered, is an \texttt{UnorderedSubList}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding an unordered list to a PDF document.},label={lis:Adding-an-unordered},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
...
\begin{marginfigure}
\includegraphics[scale=0.66]{figures/ch4/fig8}\caption{An ordered list.}\label{mar:Results-from-adding-3}
\end{marginfigure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={An ordered list.},label={lis:Adding-an-ordered},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
...
\end{itemize}\begin{marginfigure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={A combined ordered and unordered list.},label={lis:List-combining-ordered},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
using ChapterExamples.Utility;
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Using a list's GetOverFlowList method.},label={lis:overflow-Using-a-list's},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
using ChapterExamples.Utility;
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding notes to a PDF document.},label={lis:Adding-notes-to},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
using ChapterExamples.Utility;
...
\leavevmode\egroup
\begin{figure}
\includegraphics[scale=0.25]{figures/ch4/fig12}\caption{Adding notes to a PDF document.}\label{fig:Adding-notes-to}
\end{figure}
...
lets create another example illustrating page numbering labels.\begin{marginfigure}
\includegraphics[scale=0.42]{figures/ch4/fig13}\caption{Page numbering styles.}\label{fig:Different-page-numbering}
\end{marginfigure} Listing
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Page numbering styles.},label={lis:Adding-different-page},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
using ChapterExamples.Utility;
...
watermark to your PDF document. \marginnote{You must add a watermark to a Template if you wish it to appear on multiple pages.}  Using the class is straight-forward.
However, note that the watermark is added to a page and not a document.
You must add the watermark to each page of your document or use a \texttt{Template}. Listing
...
\end{figure}
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding a text watermark to a PDF document.},label={lis:watermark-Adding-a-text},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
using ChapterExamples.Utility;
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Using AutoLayout to create a PDF document.},label={lis:autolayout},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
using ChapterExamples.Utility;
...
\leavevmode\egroup
\begin{figure}
\includegraphics[scale=0.32]{figures/ch4/fig17}\caption{Using AutoLayout to create a PDF.}\label{fig:autolayout}
\end{figure}
\section{Summary}
...
\begin{marginfigure}
\includegraphics[scale=0.45]{figures/ch5/fig6}\caption{Different line style in a PDF document.}\label{mar:Different-line-style}
\end{marginfigure}
...
\begin{marginfigure}
\includegraphics[scale=0.215]{figures/ch5/fig1}\caption{Lines added to a PDF document.}\label{mar:Lines-added-to}
\end{marginfigure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Using the Line class.},label={lis:Using-the-Line},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
...
\begin{marginfigure}
\includegraphics[scale=0.185]{figures/ch5/fig2}\caption{Three rectangles added to a PDF document.}\label{mar:Adding-two-rectangles}
\end{marginfigure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding two rectangles to a PDF document.},label={lis:Adding-two-rectangles},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
using ChapterExamples.Utility;
...
\begin{figure}
\includegraphics[scale=0.38]{figures/ch5/fig5}\caption{Adding circles to a PDF document.}\label{mar:Adding-a-circle}
\end{figure}
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding circles to a PDF document.},label={lis:Adding-circles-to},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
using ChapterExamples.Utility;
...
\begin{figure}
\includegraphics[scale=0.51]{figures/ch5/fig7a}\caption{Adding a triangle to a PDF document.}\label{mar:a-Adding-a-triangle}
\end{figure}
...
\begin{figure}
\includegraphics[scale=0.35]{figures/ch5/fig7b}\caption{Creating a shape using a path.}\label{mar:a-Creating-a-shape}
\end{figure}
...
\begin{figure}
\includegraphics[scale=0.37]{figures/ch5/fig7c}\caption{Adding a path to PDF document.}\label{mar:b-Adding-a-path}
\end{figure}

\begin{figure}
\includegraphics[scale=0.30]{figures/ch5/fig7}\caption{Completed PDF document.}\label{fig:Steps-to-adding}
\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding path to a PDF document.},label={lis:Adding-path-to},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
using ChapterExamples.Utility;
...
\begin{figure}
\includegraphics[scale=0.70]{figures/ch5/fig8}\caption{Using AutoLayout to create a PDF document.}\label{mar:autolayout-shape}
\end{figure}
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Using AutoLayout to create a PDF document.},label={lis:autolayout-shapes},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
...
\begin{marginfigure}
\includegraphics[scale=0.17]{figures/ch6/egypt12dpi}\caption{Changing the DPI from 72 to 12 for an image.}\label{mar:Changing-the-DPI}
\end{marginfigure}
...
from 72 to 12 dpi.
\begin{marginfigure}
\includegraphics[scale=0.18]{figures/ch6/2024-08-26_15-35-17}\caption{Viewing an image's properties in GIMP.}\label{mar:gimpViewing-an-image's}
\end{marginfigure}
...
\begin{figure}
\includegraphics[scale=0.188]{figures/ch6/fig8}\caption{Modifying image dimensions.}\label{fig:Modifying-dimensions-of}
\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding images to a PDF document.},label={lis:Adding-images-to},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
...
\begin{marginfigure}
\includegraphics[scale=0.17]{figures/ch6/fig12}\caption{Adding a JPEG using the JpegImageData class.}\label{mar:Adding-a-jpeg}
\end{marginfigure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding an image using image's data.},label={lis:imagedata-jpg-Adding-an-Image},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.Imaging;
using ceTe.DynamicPDF.PageElements;
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Handling TIFFs.},label={lis:tif-Example-illustrating-handling},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Creating a document from a TIFF.},label={lis:tif-Creating-a-document},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{figure}
\includegraphics[scale=0.16]{figures/ch6/fig10}\caption{Adding an image from TIFF to a page.}\label{mar:tiff-Adding-an-image}
\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding a TIFF's image to a page.},label={lis:Adding-a-tiff's},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{figure}
\includegraphics[scale=0.16]{figures/ch6/fig11}\caption{Creating a new PDF document from a multi-page TIFF.}\label{mar:multipage-tiffCreating-a-new}
\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Creating a document from multi-page TIFF.},label={lis:tiff-multi-Creating-a-new},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{figure}
\includegraphics[scale=0.16]{figures/ch6/fig9}\caption{Adding images from a multi-page TIFF to a PDF document.}\label{mar:mul-tif-Adding-images-from}
\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding multi-page tiff images to pages.},label={lis:tif-Adding-images-from},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding a background image.},label={lis:Adding-a-background},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
using ChapterExamples.Utility;
...
\leavevmode\egroup
\begin{figure}
\includegraphics[scale=0.46]{figures/ch6/fig6}\caption{Adding a background image to a PDF document.}\label{fig:Adding-a-background}
\end{figure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding a watermark.},label={lis:Adding-a-watermark},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
using ChapterExamples.Utility;
...
\begin{figure}
\includegraphics[scale=0.29]{figures/ch6/fig4}\caption{Adding a watermark to a PDF document.}\label{mar:Adding-a-watermark}
\end{figure}
...
\includegraphics[scale=0.30]{figures/ch6/fig13}\caption{Adobe Acrobat displaying a list of available views.}\label{mar:Adobe-Acrobat-displaying}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding a 3D image.},label={lis:image3Adding-a-3D},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.Imaging;
using ceTe.DynamicPDF.PageElements;
...
\begin{figure}[h]
\includegraphics[scale=0.22]{figures/ch6/fig1}\caption{Results of adding and manipulating a 3D object on PDF document.}\label{fig:3d-Results-of-adding}
\end{figure}
\section{AutoLayout}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Using AutoLayout to create a PDF document.},label={lis:autolayout-images},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{marginfigure}
\includegraphics[scale=0.50]{figures/ch7/fig6}\caption{Content area added to PDF.}\label{mar:Content-area-added}
\end{marginfigure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Content area added to PDF.},label={lis:Content-area-added},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding area group to a PDF.},label={lis:Adding-area-group},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{figure}
\includegraphics[scale=0.48]{figures/ch7/fig5}\caption{Area group added to a PDF.}\label{fig:Area-group-added}
\end{figure}
...
\begin{figure}
\includegraphics[scale=0.47]{figures/ch7/fig4}\caption{Anchor groups added to PDF.}\label{fig:Anchor-groups-added}
\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding anchor groups to PDF.},label={lis:Adding-anchor-groups},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{marginfigure}
\includegraphics[scale=0.15]{figures/ch7/fig3}\caption{PDF document with a TransformationGroup applied.}\label{mar:PDF-document-with}
\end{marginfigure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Applying a TransformationGroup.},label={lis:Applying-a-TransformationGroup},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Applying transparency to several page elements.},label={lis:Applying-transparency-to},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\includegraphics[scale=0.36]{figures/ch7/fig2}\caption{PDF document with transparency applied to several page elements.}\label{fig:PDF-document-with}
...
\begin{table}[h]
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}} l p{2.0in}}
\toprule 
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={PDF document using Core Fonts.},label={lis:PDF-document-using},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{figure}
\includegraphics[scale=0.35]{figures/ch8/fig1}\caption{PDF document using Core Fonts.}\label{fig:PDF-document-using}
...
PDF document.
\begin{figure}
\includegraphics[scale=0.38]{figures/ch8/fig3}\caption{PDF document using Google fonts.}\label{fig:google-PDF-document-using-1}
\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={PDF document using Google fonts.},label={lis:google-PDF-document-using-1},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{figure}
\includegraphics[scale=0.36]{figures/ch8/fig2}\caption{Incorrect Google Font setting.}\label{mar:Incorrect-Google-Font}
...
\includegraphics[scale=0.53]{figures/ch8/fig4}\caption{PDF document using OpenType fonts.}\label{fig:opentype-PDF-document-using-1}

\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={PDF document using OpenType Font.},label={lis:opentype-PDF-document-using-1},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Embedding a font.},label={lis:Embedding-a-font.},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Encoding a PDF document.},label={lis:Encoding-a-PDF},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{figure}
\includegraphics[scale=0.83]{figures/ch8/fig5}\caption{Encoding a PDF in Czech.}\label{mar:Encoding-a-PDF}
\end{figure}
...
\begin{marginfigure}
\includegraphics[scale=0.42]{figures/ch8/fig6}\caption{Kerning and Tracking applied to a PDF.}\label{mar:Kerning-and-Tracking}
\end{marginfigure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Kerning and tracking applied to a PDF.},label={lis:Kerning-and-tracking},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Colors in a PDF document.},label={lis:Colors-in-a},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
...
\begin{figure}
\includegraphics[scale=0.42]{figures/ch8/fig8}\caption{Gradients in a PDF document.}\label{fig:Gradients-in-a}
\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Gradients in a PDF document.},label={lis:Gradients-in-a},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Using spot color in a PDF document.},label={lis:Using-spot-color},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={A simple table example.},label={lis:A-simple-table},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Spanning rows and columns.},label={lis:Spanning-rows-and},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding formatting to a table.},label={lis:Adding-formatting-to},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
...
\begin{figure}
\includegraphics[scale=0.65]{figures/ch9/fig4}\caption{Formatting a table's borders.}\label{fig:fig-Formatting-a-table's}
\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Formatting a table's borders.},label={lis:Formatting-a-table's},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Table formatting precedence.},label={lis:Table-formatting-precedence.},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={A basic table.},label={lis:A-basic-table.},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
...
\begin{figure}
\includegraphics[scale=0.20]{figures/ch9/fig6}\caption{Example of table overflow.}\label{fig:overflow-table}
\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Example of table overflow in a table.},label={lis:overflow-Example-of-table},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
...
\begin{lstlisting}[caption={An example of column overflow in a table.},label={lis:col-over-An-example-of},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
...
\begin{figure}
\includegraphics[scale=0.20]{figures/ch9/fig7}\caption{An example of column overflow in a table.}\label{fig:col-overflow-An-example-of}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding page elements to table cells.},label={lis:cells-Adding-page-elements},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
...
\includegraphics[scale=0.38]{figures/ch9/fig5}\caption{Adding page elements to table cells.}\label{fig:fig-Adding-page-elements}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Using AutoLayout to create a PDF document.},label={lis:autolayout-table},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
...
title. 
\begin{marginfigure}
\includegraphics[scale=0.20]{figures/ch10/fig1}\caption{A simple pie chart.}\label{mar:A-simple-pie}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Creating a simple chart.},label={lis:chart-simple-create},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
...
\marginnote{Do not be confused if you see code referring to a plot area by a Chart
...
\end{figure}
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding a plot area to a chart.},label={lis:Adding-a-plot},basicstyle={\footnotesize\ttfamily},breaklines=true]
private static void AddPlot(Chart chart, float wdth, float hght)
{
...
changed.
\begin{figure}
\includegraphics[scale=0.50]{figures/ch10/fig3}\caption{Customizing a legend.}\label{fig:Customizing-a-legend.}

\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Customizing a legend.},label={lis:Customizing-a-legend.},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={A pie chart example.},label={lis:A-pie-chart},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements.Charting;
using ceTe.DynamicPDF.PageElements.Charting.Series;
...
{\scriptsize\texttt{Indexed100PercentStacked\-BarSeries}} & {\scriptsize\texttt{Indexed100PercentStacked\-BarSeriesElement}}\tabularnewline
{\scriptsize\texttt{DateTime100PercentStacked\-BarSeries}} & {\scriptsize\texttt{DateTime100PercentStacked\-BarSeriesElement}}\tabularnewline
\bottomrule
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={A simple bar chart with indexed data.},label={lis:A-simple-bar},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements.Charting;
...
\end{figure}
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={A simple bar chart with date/time data.},label={lis:A-simple-bar-datetime1},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements.Charting;
using ceTe.DynamicPDF.PageElements.Charting.Axes;
using ceTe.DynamicPDF.PageElements.Charting.Series;
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Stacked bar chart with indexed data example.},label={lis:Stacked-bar-chart},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements.Charting;
using ceTe.DynamicPDF.PageElements.Charting.Axes;
using ceTe.DynamicPDF.PageElements.Charting.Series;
...
\end{figure}
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={A date/time stacked bar chart.},label={lis:A-date/time-stacked},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements.Charting;
using ceTe.DynamicPDF.PageElements.Charting.Axes;
using ceTe.DynamicPDF.PageElements.Charting.Series;
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={A stacked 100\% indexed bar chart.},label={lis:A-stacked-100=000025},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements.Charting;
using ceTe.DynamicPDF.PageElements.Charting.Axes;
using ceTe.DynamicPDF.PageElements.Charting.Series;
...
\end{figure}
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Stacked 100\% data/time bar chart.},label={lis:Stacked-100=000025-data/time},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements.Charting;
using ceTe.DynamicPDF.PageElements.Charting.Axes;
using ceTe.DynamicPDF.PageElements.Charting.Series;
...
{\scriptsize\texttt{Indexed100PercentStacked\-ColumnSeries}} & {\scriptsize\texttt{Indexed100PercentStacked\-ColumnSeriesElement}}\tabularnewline
{\scriptsize\texttt{DateTime100PercentStacked\-ColumnSeries}} & {\scriptsize\texttt{DateTime100PercentStacked\-ColumnSeriesElement}}\tabularnewline
\bottomrule
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Column chart with indexed data.},label={lis:Column-chart-with},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements.Charting;
using ceTe.DynamicPDF.PageElements.Charting.Axes;
using ceTe.DynamicPDF.PageElements.Charting.Series;
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Stacked column chart of indexed data.},label={lis:Stacked-column-chart},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF.PageElements.Charting;
using ceTe.DynamicPDF.PageElements.Charting.Axes;
using ceTe.DynamicPDF.PageElements.Charting.Series;
using System;
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Property precedence using bar chart.},label={lis:Property-precedence-using},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements.Charting;
using ceTe.DynamicPDF.PageElements.Charting.Axes;
using ceTe.DynamicPDF.PageElements.Charting.Series;
...
\begin{figure}
\includegraphics[scale=0.41]{figures/ch10/fig17}\caption{Auto-layout example.}\label{mar:Auto-layout-example.}

\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Auto-layout example.},label={lis:Auto-layout-example.},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{marginfigure}
\includegraphics[scale=0.21]{figures/ch10/fig18}\caption{Chart with auto-layout disabled without formatting.}\label{mar:Chart-with-auto-layout}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Manually setting plot area and legend.},label={lis:Manually-setting-plot},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements.Charting;
using ceTe.DynamicPDF.PageElements.Charting.Series;
...
\begin{figure}
\includegraphics[scale=0.42]{figures/ch10/fig19}\caption{Manually laying out a pie chart.}\label{fig:Manually-laying-out}
\end{figure}
...
\begin{figure}
\includegraphics[scale=0.40]{figures/ch10/fig21}\caption{Modifying a chart's layout manually.}\label{fig:Modifying-a-chart's}
\end{figure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Modifying a chart's layout manually.},label={lis:Modifying-a-chart's},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements.Charting;
using ceTe.DynamicPDF.PageElements.Charting.Series;
...
PDF document.
\begin{figure}[h]
\includegraphics[scale=0.54]{figures/ch10/fig20}\caption{A chart with two plot areas.}\label{fig:two-plot-A-chart-with}
\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={A chart with two plot areas.},label={lis:two-plot-A-chart-with},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements.Charting;
using ceTe.DynamicPDF.PageElements.Charting.Axes;
using ceTe.DynamicPDF.PageElements.Charting.Series;
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Using AutoLayout to create a PDF document.},label={lis:autolayout-chart},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
using ceTe.DynamicPDF.PageElements.Charting;
using ceTe.DynamicPDF.PageElements.Charting.Series;
...
{\scriptsize\texttt{Indexed100PercentStacked\-LineSeries}} & {\scriptsize\texttt{Indexed100PercentStackedLine\-SeriesElement}}\tabularnewline
{\scriptsize\texttt{DateTime100PercentStacked\-LineSeries}} & {\scriptsize\texttt{DateTime100PercentStackedLine\-SeriesElement}}\tabularnewline
\bottomrule
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Line chart using indexed data.},label={lis:Line-indx-chart-using},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements.Charting;
using ceTe.DynamicPDF.PageElements.Charting.Axes;
using ceTe.DynamicPDF.PageElements.Charting.Series;
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Line chart using date/time data.},label={lis:Line-dt-chart-using},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements.Charting;
using ceTe.DynamicPDF.PageElements.Charting.Axes;
using ceTe.DynamicPDF.PageElements.Charting.Series;
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Stacked indexed line chart.},label={lis:Stacked-indexed-line-chart},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements.Charting;
using ceTe.DynamicPDF.PageElements.Charting.Axes;
using ceTe.DynamicPDF.PageElements.Charting.Series;
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Stacked date/time line chart.},label={lis:Stacked-date/time-line},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements.Charting;
using ceTe.DynamicPDF.PageElements.Charting.Axes;
using ceTe.DynamicPDF.PageElements.Charting.Series;
...
resulting chart.
\begin{figure}
\includegraphics[scale=0.45]{figures/ch11/fig5}\caption{Stacked 100\% indexed line chart.}\label{fig:line-Stacked-100=000025-indexed}

\end{figure}
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Stacked 100\% indexed line chart.},label={lis:line-Stacked-100=000025-indexed},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements.Charting;
using ceTe.DynamicPDF.PageElements.Charting.Axes;
using ceTe.DynamicPDF.PageElements.Charting.Series;
...
\end{figure}
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Stacked 100\% date/time line chart.},label={lis:dt-Stacked-100=000025-date/time},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements.Charting;
using ceTe.DynamicPDF.PageElements.Charting.Axes;
using ceTe.DynamicPDF.PageElements.Charting.Series;
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Area chart using indexed data.},label={lis:indexded-Area-chart-using},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements.Charting;
using ceTe.DynamicPDF.PageElements.Charting.Axes;
using ceTe.DynamicPDF.PageElements.Charting.Series;
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Stacked area chart with date/time data.},label={lis:Stacked-area-chart},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements.Charting;
using ceTe.DynamicPDF.PageElements.Charting.Axes;
using ceTe.DynamicPDF.PageElements.Charting.Series;
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Scatter chart.},label={lis:Scatter-chart.},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements.Charting;
using ceTe.DynamicPDF.PageElements.Charting.Series;
...
chart.
\begin{figure}
\includegraphics[scale=0.45]{figures/ch11/fig10}\caption{Grid-lines and tick-marks on a line chart.}\label{fig:Grid-lines-and-tick-marks}
\end{figure}
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Gridlines and tick-marks on a line chart.},label={lis:Gridlines-and-tick-marks},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{figure}
\includegraphics[scale=0.45]{figures/ch11/fig12}\caption{Modifying axes labels.}\label{mar:Modifying-the-x-axis}
\end{figure}
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Modifying axes labels.},label={lis:Modifying-axes-labels.},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
the resulting chart.
\begin{figure}[h]
\includegraphics[scale=0.35]{figures/ch11/fig14}\caption{Adding markers to a line chart.}\label{fig:Adding-markers-to-1}
\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding markers to a series.},label={lis:Adding-markers-to},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding markers to a scatter plot.},label={lis:scatterplot-Adding-markers-to-1},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{figure}[h]
\includegraphics[scale=0.40]{figures/ch11/fig17}\caption{Adding a scalar data label to a pie chart.}\label{mar:data-lable:Adding-a-scalar}
\end{figure}
...
\begin{figure}
\includegraphics[scale=0.45]{figures/ch11/fig15}\caption{Modifying a line series labels.}\label{mar:Modifying-a-line}
\end{figure}
\bgroup\inputencoding{latin9}
\begin{lstlisting}[basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{figure}[h]
\includegraphics[scale=0.45]{figures/ch11/fig13}\caption{Adding page elements to a page containing a chart.}\label{fig:chart-Adding-page-elements}

\end{figure}
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding page elements to a chart.},label={lis:chart-Adding-page-elements},basicstyle={\footnotesize\ttfamily},breaklines=true]
doc.Pages[0].Elements.Add(chart);
...
to a PDF document.
\begin{marginfigure}
\includegraphics[scale=0.5]{figures/ch12/fig1}\caption{UPC codes added to a PDF document.}\label{fig:UPC-codes-added}
\end{marginfigure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={UPC codes added to a PDF document.},label={lis:UPC-codes-added},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
...
\begin{figure}
\includegraphics[scale=0.7]{figures/ch12/fig4}\caption{EAN codes added to a PDF document.}\label{mar:EAN-codes-added}
\end{figure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding EAN codes to a PDF document.},label={lis:Adding-EAN-codes},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
using ceTe.DynamicPDF.PageElements.BarCoding;
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding a QR code to a PDF document.},label={lis:Adding-a-QR},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements.BarCoding;
namespace ChapterExamples.Chapters.ChapterTwelve
{
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding an Aztec code to a PDF document.},label={lis:Adding-an-Aztec},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements.BarCoding;
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Using AutoLayout to create a PDF document.},label={lis:autolayout-barcode},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
using ceTe.DynamicPDF.PageElements.BarCoding;
...
\begin{figure}
\includegraphics[scale=0.25]{figures/ch13/fig10}\caption{A PDF with outline.}\label{fig:outline-A-PDF-with}
...
\begin{marginfigure}
\includegraphics[scale=0.50]{figures/ch13/fig8}\caption{A PDF document with child outlines.}\label{mar:child-outlinesA-PDF-document}
\end{marginfigure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding an outline.},label={lis:Adding-and-outline},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
using ChapterExamples.Utility;
...
\begin{figure}
\includegraphics[scale=0.40]{figures/ch13/fig1}\caption{Adding an outline to a PDF Document.}\label{fig:Adding-an-outline}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={PDF document with child outlines.},label={lis:child-PDF-document-with},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
using ChapterExamples.Utility;
...
\leavevmode\egroup
\begin{figure}[h]
\includegraphics[scale=0.40]{figures/ch13/fig2}\caption{Adding an outline with child outlines to a PDF document.}\label{fig:chiled-Adding-an-outline-1}
\end{figure}
...
\begin{figure}[h]
\includegraphics[scale=0.38]{figures/ch13/fig3}\caption{Adding multiple outlines to a PDF document.}\label{fig:Adding-multiple-outlines}
\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Multiple outlines.},label={lis:multiple-out-A-PDF-document-1},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
...
initial zoom percent (\texttt{InitialPageZoomPercentage}\index{InitialPageZoomPercentage}), page
...
illustrates the resulting PDF document.
\begin{figure}
\includegraphics[scale=0.20]{figures/ch13/fig4}\caption{Adding an outline with actions to a PDF document.}\label{fig:actions-:Adding-an-outline-1}
\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Outline actions.},label={lis:illus-out-acti-A-PDF-document-1},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
...
in a PDF document and belongs to the \texttt{ceTe.DynamicPDF\allowbreak.PageElements}\index{ceTe.DynamicPDF.PageElements}
...
\begin{figure}
\includegraphics[scale=0.45]{figures/ch13/fig5}\caption{PDF document with bookmarks.}\label{fig:bookmarks-PDF-document-with-1}
\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={PDF document with bookmarks.},label={lis:bookmarkes=00003DPDF-document-with},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
using ChapterExamples.Utility;
...
\begin{marginfigure}
\includegraphics[scale=0.50]{figures/ch13/fig9}\caption{Bookmarks and Outlines.}\label{mar:Bookmarks-and-Outlines.}
\end{marginfigure}
...
illustrates the resulting PDF document.
\begin{figure}
\includegraphics[scale=0.42]{figures/ch13/fig6}\caption{PDF document with outline and bookmarks.}\label{fig:outline-bookmarks-PDF-document-with-1}
\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Pdf with outline and bookmarks.},label={lis:outline-book-Pdf-document-with},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
using ChapterExamples.Utility;
...
\toprule 
 ...
 & {\tiny\texttt{RC440Security}} & {\tiny\texttt{RC4128Security}} & {\tiny\texttt{Aes128Security}} & {\tiny\texttt{Aes256Security}}\tabularnewline
\midrule 
...
\begin{marginfigure}
\includegraphics[scale=0.25]{figures/ch14/fig1}\caption{A password protected PDF document.}\label{mar:A-password-protected}
\end{marginfigure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Encrypting and signing a PDF document.},label={lis:Encrypting-and-signing},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
...
\end{lstlisting}
\leavevmode\egroup
Printing using the PDF created with \texttt{Rc440Example}\index{Rc440Example} requires
the user to enter the owner password or they are not allowed to print (Figure
\ref{fig:printinig-disabled-A-PDF-document}).\begin{marginfigure}
\includegraphics[scale=0.34]{figures/ch14/fig2}\caption{A PDF document with printing disabled.}\label{fig:printinig-disabled-A-PDF-document}
\end{marginfigure} Attempting to edit the PDF created with \texttt{Aes256Example}\index{Aes256Example} requires the user to enter the owner password or they are prevented from editing
the document. And finally, Figure \ref{fig:annotations-disabledA-PDF-document}
illustrates the effect of disabling annotations on the PDF created
using \texttt{Aes256Example}\index{Aes256Example}.\begin{marginfigure}
\includegraphics[scale=0.34]{figures/ch14/fig14}\caption{Attempting to edit a protected PDF.}\label{mar:protected-edit-Attempting-to-edit}
\end{marginfigure}  Note that annotations are entirely disabled
and there is no option to enter the owner password to allow annotations.
\begin{figure}
\includegraphics[scale=0.28]{figures/ch14/fig3}\caption{A PDF document with annotations disabled.}\label{fig:annotations-disabledA-PDF-document}
\end{figure}
...
\marginnote{There are numerous online resources providing instructions on creating
self-signed certificates. For example, https://learn.microsoft.com/en-us/entra/identity-platform/howto-create-self-signed-certificate}
...
authority, not a self-signed certificate as we do here. But these
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Certify method signatures.},label={lis:Certify-method-signatures.},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{lstlisting}[caption={Certifying a PDF Document.},label={lis:Certifying-a-PDF},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
...
\begin{figure}
\includegraphics[scale=0.27]{figures/ch14/fig4}\caption{Digitally certified PDF document.}\label{mar:Digitally-certified-PDF}
\end{figure}
...
\includegraphics[scale=0.40]{figures/ch14/fig13}\caption{A digital signature.}\label{mar:panel-A-digital-signature.}
...
\begin{figure}
\includegraphics[scale=0.30]{figures/ch14/fig8}\caption{Digitally signed PDF document.}\label{fig:Digitally-signed-PDF}
\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Signing a PDF document.},label={lis:Signing-a-PDF},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements.Forms;
...
signature's properties and validity status by clicking on the certificate.\begin{marginfigure}
\includegraphics[scale=0.14]{figures/ch14/fig10}\caption{Viewing a signature's signature panel.}\label{fig:Viewing-a-signature's}
\end{marginfigure}
...
method to the code in Listing \ref{lis:Signing-a-PDF}.\begin{marginfigure}
\includegraphics[scale=0.40]{figures/ch14/fig12}\caption{An image used as a signature.}\label{mar:sig-An-image-used}
\end{marginfigure} The code is
...
signature. 
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Signing a PDF document.},label={lis:img-=000020Signing-a-PDF-1},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
Party (TTP) acting as a Time Stamping Authority (TSA).\begin{marginfigure}
\includegraphics[scale=0.3]{figures/ch14/fig9}\caption{Timestamp information for a PDF document.}\label{mar:Timestamp-information-for}
\end{marginfigure} When a trusted
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding a timestamp to a PDF document's signature.},label={lis:Adding-a-timestamp},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\marginnote{Make sure to use form elements from the ceTe\allowbreak.DynamicPDF.PageElements.Forms namespace when creating forms.}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Creating a CheckBox.},label={lis:Creating-a-CheckBox.},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{marginfigure}
\includegraphics[scale=0.40]{figures/ch15/fig2}\caption{A RadioButton element.}\label{mar:A-RadioButton-element.}
\end{marginfigure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Creating a RadioButton.},label={lis:Creating-a-RadioButton.},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\section{Text Fields}
\begin{marginfigure}
\includegraphics[scale=0.43]{figures/ch15/fig3}\caption{A TextField element.}\label{mar:A-TextField-element.}
\end{marginfigure}
...
fields specific to text elements.\begin{marginfigure}
\includegraphics[scale=0.43]{figures/ch15/fig4}\caption{A multi-line TextField element.}\label{mar:A-multi-line-TextField}
\end{marginfigure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Creating a TextField element.},label={lis:Creating-a-TextField},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Creating a multi-line TextField element.},label={lis:Creating-a-multi-line},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
to a PDF document (Figure \ref{mar:A-ListBox-element.}). First, you create a \texttt{ListBox}\index{ListBox} instance. You then add the weekdays
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Creating a ListBox element.},label={lis:Creating-a-ListBox},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{marginfigure}
\includegraphics[scale=0.45]{figures/ch15/fig6}\caption{A ComboBox element.}\label{mar:A-ComboBox-element.}
\end{marginfigure}
...
\includegraphics[scale=0.38]{figures/ch15/fig13}\caption{Selecting a choice or typing a new choice.}\label{fig:Selecting-a-choice}

\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Creating a ComboBox element.},label={lis:Creating-a-ComboBox},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{marginfigure}
\includegraphics[scale=0.6]{figures/ch15/fig7}\caption{Two Button elements.}\label{mar:A-Button-element.}

\end{marginfigure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Creating two Button elements.},label={lis:Creating-two-Button},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{figure}
\includegraphics[scale=0.43]{figures/ch15/fig8}\caption{The complete form.}\label{fig:The-complete-form.}
...
\begin{marginfigure}
\includegraphics[scale=0.35]{figures/ch15/figure14}\caption{A button with images for different behavior.}\label{mar:behavior-pushA-button-with}
\end{marginfigure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={A button with images.},label={lis:A-button-with},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.Imaging;
using ceTe.DynamicPDF.PageElements.Forms;
...
\begin{figure}
\includegraphics[scale=0.56]{figures/ch15/fig9}\caption{A Signature element.}\label{mar:A-Signature-element.}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Creating a simple form with a Signature element.},label={lis:sig-Creating-a-simple},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
using ceTe.DynamicPDF.PageElements.Forms;
...
\begin{figure}
\includegraphics[scale=0.2]{figures/ch15/fig10}\caption{Signing a completed form.}\label{fig:Signing-a-completed}
...
\begin{figure}
\includegraphics[scale=0.20]{figures/ch15/fig12}\caption{Opening the signed document.}\label{fig:Opening-the-signed}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Using AutoLayout to create a PDF document.},label={lis:autolayout-forms},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
event is fired and can be handled by one or more actions (\bgroup\inputencoding{latin9}\lstinline[basicstyle={\footnotesize\ttfamily},breaklines=true]!document.ReaderEvents.Open = <an-action>;!\egroup
...
\begin{figure}
\includegraphics[scale=0.30]{figures/ch16/fig1}\caption{Document events - printing a PDF document.}\label{fig:Document-events.}
\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Document events.},label={lis:Document-events.},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Page events.},label={lis:Page-events.},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{figure}
\includegraphics[scale=0.35]{figures/ch16/fig2}\caption{Page events.}\label{fig:Page-events.}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Form field events.},label={lis:Form-field-events.},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{figure}
\includegraphics[scale=0.20]{figures/ch16/fig3}\caption{Form field events.}\label{fig:Form-field-events.}
\end{figure}
...
\begin{marginfigure}
\includegraphics[scale=0.15]{figures/ch16/fig4}\caption{Hiding and showing a form field.}\label{fig:Hiding-and-showing}
\end{marginfigure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Hiding and showing a form field.},label={lis:Hiding-and-showing},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Navigating to a URL using UrlAction.},label={lis:url-Navigating-to-a},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Navigating to page using GoToAction.},label={lis:Navigating-to-page},basicstyle={\footnotesize\ttfamily},breaklines=true,showstringspaces=false]
...
Open a file using the \texttt{FileOpenAction}\index{FileOpenAction} action.\begin{marginfigure}
\includegraphics[scale=0.30]{figures/ch16/fig6}\caption{Opening a file with \texttt{FileOpenAction}\index{FileOpenAction}.}\label{mar:Opening-a-file}
\end{marginfigure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Opening a file with FileOpenAction.},label={lis:Opening-a-file},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
(Listing \ref{lis:Resetting-a-form's}).\begin{marginfigure}
\includegraphics[scale=0.18]{figures/ch16/fig7}\caption{Resetting a form's values using ResetAction.}\label{mar:Resetting-a-form's}
\end{marginfigure} For example, when the button
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Resetting a form's values using ResetAction.},label={lis:Resetting-a-form's},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{marginfigure}
\includegraphics[scale=0.30]{figures/ch16/fig9}\caption{Importing form data.}\label{mar:form-fill}
\end{marginfigure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Importing form data.},label={lis:import-form-data},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Chaining multiple actions in a PDF document.},label={lis:Chaining-multiple-actions},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{marginfigure}
\includegraphics[scale=0.30]{figures/ch17/fig12}\caption{JavaScript method that requires forms rights.}\label{mar:forms-rights-JavaScript-method-that}
...
to the \texttt{Document} instance's \texttt{JavaScripts}\index{JavaScripts} collection.\begin{marginfigure}
\includegraphics[scale=0.35]{figures/ch17/fig1}\caption{Accessing DOM using JavaScript.}\label{mar:Accessing-DOM-using}
\end{marginfigure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Accessing Document object in Adobe Acrobat using JavaScript.},label={lis:Accessing-Doc-object},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
...
and so it is executed by the PDF document upon being opened.
\begin{lstlisting}[basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{marginfigure}
\includegraphics[scale=0.5]{figures/ch17/fig10}\caption{A PDF document where changing the ComboBox updates TextBox.}\label{mar:changing-combo-A-PDF-document}
\end{marginfigure}
...
by the combo box (Figure \ref{mar:changing-combo-A-PDF-document}). It uses the \texttt{ComboBox}\index{ComboBox} element's \texttt{OnBlur}\index{OnBlur} event to
execute the JavaScript that changes the \texttt{TextField}\index{TextField} element's
text.

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Javascript to change a field value.},label={lis:Javascript-to-change},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={JavaScript that sums two values.},label={lis:JavaScript-that-sums},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{marginfigure}
\raggedright
\includegraphics[scale=0.43]{figures/ch17/fig13}\caption{Web page displaying date and time.}\label{mar:Web-page-displaying}

\end{marginfigure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={A webpage with JavaScript.},label={lis:A-webpage-with},basicstyle={\footnotesize\ttfamily},breaklines=true]
<html>
<script>
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={JavaScript function to validate age.},label={lis:age-JavaScript-function-to},basicstyle={\footnotesize\ttfamily},breaklines=true]
function numonly(){
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Using document level function.},label={lis:function-Using-document-level},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
the following hyperlink in an HTML document (\texttt{<a href=\textquotedbl mailto:user@example\allowbreak.com\textquotedbl >Send
...
\end{itemize}
\begin{marginfigure}
\includegraphics[scale=0.14]{figures/ch18/fig10}\caption{Emailing an XML form's data.}\label{mar:Emailing-an-XML}
\end{marginfigure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Using JavaScript to email a form.},label={lis:email-form-Using-JavaScript-to},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
using ceTe.DynamicPDF.PageElements.Forms;
...
\begin{figure}
\includegraphics[scale=0.34]{figures/ch18/fig1}\caption{Emailing form data.}\label{fig:Emailing-form-data.}
\end{figure}
...
(Figure \ref{mar:Emailing-form-data.}, Listing \ref{lis:fdf-Email-response-formatted}).


\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Email attachment formatted as FDF.},label={lis:fdf-Email-response-formatted},basicstyle={\footnotesize\ttfamily},breaklines=true,showspaces=true,extendedchars=true]
%FDF-1.2
%����
1 0 obj
...
\begin{marginfigure}
\includegraphics[scale=0.450]{figures/ch18/fig3}\caption{Error when a server returns HTML/text.}\label{mar:html-text-Error-when-a}
...
\marginnote{PHP has a library for working with FDF to create more advanced responses. }
...
a success message (Listing \ref{lis:An-FDF-message}). 
...
\begin{marginfigure}
\includegraphics[scale=0.38]{figures/ch18/fig9}\caption{Status message in Adobe Acrobat.}\label{mar:Status-message-in}

\end{marginfigure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={An FDF message displaying success.},label={lis:An-FDF-message},basicstyle={\footnotesize\ttfamily},breaklines=true]
%FDF-1.2
1 0 obj
<<
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={A simple PHP file to return FDF success message.},label={lis:A-simple-PHP},basicstyle={\footnotesize\ttfamily},breaklines=true]
<?php
...
\begin{figure}
\includegraphics[scale=0.35]{figures/ch18/fig5}\caption{Submitting a form.}\label{fig:Submitting-a-form.}
\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Submitting form using HTTP POST.},label={lis:POST-Submitting-form-using},basicstyle={\footnotesize\ttfamily},breaklines=true,showstringspaces=false]
...
\begin{figure}
\includegraphics[scale=0.5]{figures/ch18/fig4}\caption{The request and response of form submission using HTTP POST.}\label{fig:The-request-and}
...
\begin{figure}
\includegraphics[scale=0.3]{figures/ch18/fig6}\caption{Request and responses from submitting form.}\label{fig:submitting-form-Request-and-responses}
...
files from within your PDF reader software.\marginnote{I am assuming Adobe Acrobat Reader as the PDF reader software.} 
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Embedding files as attachments.},label={lis:Embedding-files-as},basicstyle={\footnotesize\ttfamily},breaklines=true,showstringspaces=false]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
...
\begin{figure}
\includegraphics[scale=0.36]{figures/ch19/fig1}\caption{Files Embedded in a PDF document.}\label{fig:Files-Embedded-in}
...
\begin{marginfigure}
\includegraphics[scale=0.25]{figures/ch19/fig4}\caption{Dialog asking how to handle a file.}\label{mar:Dialog-asking-how}
\end{marginfigure}
...
\begin{figure}
\includegraphics[scale=0.30]{figures/ch19/fig3}\caption{Detailed and Tile attachment layouts.}\label{fig:Detailed-and-Tile}
\end{figure}
...
\begin{figure}
\includegraphics[scale=0.40]{figures/ch19/fig2}\caption{A PDF portfolio.}\label{fig:A-PDF-portfolio.}
\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Creating a PDF Portfolio.},label={lis:portfolio-Creating-a-PDF-1},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
using ChapterExamples.Utility;
...
\begin{figure}[h]
\includegraphics[scale=0.14]{figures/ch19/fig6}\caption{Previewing a file in a PDF portfolio.}\label{fig:portfolio-Previewing-a-file}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding required schema to a PDF.},label={lis:Adding-required-schemas},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Embedding a font in a PDF document.},label={lis:Embedding-a-font},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
PDF documents.
\begin{marginfigure}
\includegraphics[scale=0.2]{figures/ch20/fig26}\caption{verPDF validation tool.}\label{mar:verPDF-validation-tool.}
...
\begin{figure}
\includegraphics[scale=0.22]{figures/ch20/fig1}\caption{Default tagging of a PDF.}\label{fig:Default-tagging-of}
\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding TagOptions to a PDF.},label={lis:Adding-TagOptions-to},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
icon, it displays the tags. But notice in Figure \ref{fig:Default-tagging-of} the order is incorrect. Recall that to be compliant all content must be in logical reading
order. \begin{marginfigure}
\includegraphics[scale=0.6]{figures/ch20/fig19}\caption{Adobe Acrobat Pro's Accessibility Icon.}\label{mar:Adobe-Acrobat-Pro's}
\end{marginfigure}If this PDF was read by a screen-reader, it would read the
...
\begin{figure}
\includegraphics[scale=0.18]{figures/ch20/fig3}\caption{Incorrect Acrobat Read Aloud order.}\label{fig:Incorrect-Acrobat-Read}
\end{figure}
...
\begin{marginfigure}
\includegraphics[scale=0.25]{figures/ch20/fig4}\caption{Acrobat with correct reading order.}\label{mar:Acrobat-with-correct}
\end{marginfigure}
...
\begin{figure}
\includegraphics[scale=0.22]{figures/ch20/fig27}\caption{Tagged document failing validation.}\label{fig:Tagged-document-failing}
\end{figure}
\begin{marginfigure}
\includegraphics[scale=0.21]{figures/ch20/fig28}\caption{Passing validation.}\label{mar:Passing-validation.}
\end{marginfigure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Fixing Listing \ref{lis:Adding-TagOptions-to}.},label={lis:Fixing-Listing-.},basicstyle={\footnotesize\ttfamily},breaklines=true]
public static void BasicText(string resourcePath, string outputPath)
 {
     Document doc = new();
     doc.Tag = new TagOptions();
...
and the types of tags you use.\begin{marginfigure}
\includegraphics[scale=0.5]{figures/ch20/fig18}\caption{Correct tag order.}\label{mar:Correct-tag-order.}
\end{marginfigure} Core Suite provides the \texttt{StructureElement}\index{StructureElement}
and \texttt{Tag}\index{Tag} classes to facilitate manual tagging. The way it works is you create a \texttt{StructureElement}\index{StructureElement} instance
...
\begin{figure}
\includegraphics[scale=0.32]{figures/ch20/fig5}\caption{A tagged PDF.}\label{fig:A-tagged-PDF.}
\end{figure}
 

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Tagging a PDF document.},label={lis:Tagging-a-PDF},basicstyle={\footnotesize\ttfamily},breaklines=true,showstringspaces=false]
...
\leavevmode\egroup
\begin{marginfigure}
\includegraphics[scale=0.2]{figures/ch20/fig20}\caption{Validating a PDF.}\label{mar:Validating-a-PDF.}
\end{marginfigure}Figure \ref{fig:A-tagged-PDF.} illustrates the tagged PDF document
...
objects.
\begin{figure}
\includegraphics[scale=0.34]{figures/ch20/fig8}\caption{Tagged document with attributes.}\label{fig:attrib-Tagged-document-with}
\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding attributes to a document's tags.},label={lis:Adding-attributes-to},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{marginfigure}
\includegraphics[scale=0.21]{figures/ch20/fig29}\caption{Validating a PDF.}\label{mar:Validating-a-PDF.-1}
\end{marginfigure}
...
\begin{figure}
\includegraphics[scale=0.30]{figures/ch20/fig9}\caption{A tagged document with circles.}\label{mar:circles-A-tagged-document}
\end{figure}
 

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Providing alternate text to circles on a page.},label={lis:Providing-alternate-text},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{marginfigure}
\includegraphics[scale=0.21]{figures/ch20/fig30}\caption{Validating a PDF.}\label{mar:Validating-a-PDF.-2}
\end{marginfigure}
...
\begin{figure}
\includegraphics[scale=0.2]{figures/ch20/fig10}\caption{A tagged form.}\label{fig:form-tags}
...
\begin{marginfigure}
\includegraphics[scale=0.26]{figures/ch20/fig23}\caption{Core Suite default tagging for a TextField.}\label{mar:Core-Suite-default}
\end{marginfigure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Manually tagging a Form.},label={lis:form-Manually-tagging-a},basicstyle={\footnotesize\ttfamily},breaklines=true]
public static void Create(string outputPath)
{
    Document doc = new();
    MetaData(doc);
...
\begin{figure}
\includegraphics[scale=0.30]{figures/ch20/fig25}\caption{Manually tagged form.}\label{fig:Manually-tagged-form.}
\end{figure}
...
\begin{figure}
\includegraphics[scale=0.25]{figures/ch20/fig11}\caption{PDF with template tagged.}\label{fig:PDF-with-template}
\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Template with default tagging.},label={lis:Template-with-default},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\leavevmode\egroup
\begin{figure}
\includegraphics[scale=0.65]{figures/ch20/fig12}\caption{Raw tag structure.}\label{mar:Raw-tag-structure.}
\end{figure}
...
\begin{figure}
\includegraphics[scale=0.25]{figures/ch20/fig13}\caption{A tagged document after using template.}\label{fig:A-tagged-document}
\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Identifying items as artifacts to avoid tagging.},label={lis:Identifying-items-as},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\section{Summary}
\begin{marginfigure}
\includegraphics[scale=0.21]{figures/ch20/fig31}\caption{PDF document passing validation.}\label{mar:PDF-document-passing}
\end{marginfigure}
...
\marginnote{In this book we only examine level A compliance.}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={IccProfile and OutputIntent.},label={lis:IccProfile-and-OutputIntent.},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{figure}
\includegraphics[scale=0.28]{figures/ch21/fig6}\caption{A PDF/A-1a compliant document.}\label{fig:A-PDF/A-1a-compliant}

\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Creating a PDF/A-1a PDF document.},label={lis:Creating-a-PDF/A-1a-1},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
using ceTe.DynamicPDF.Text;
using ceTe.DynamicPDF.Xmp;
...
\section{PdfA-2a}
\begin{marginfigure}
\includegraphics[scale=0.25]{figures/ch21/fig2}\caption{Passing PDFA-1a validation.}\label{mar:Passing-PDFA-1a-validation.}
\end{marginfigure}
...
\begin{marginfigure}
\includegraphics[scale=0.25]{figures/ch21/fig4}\caption{Passing PDF/A-2a validation.}\label{mar:Passing-PDF/A-2a-validation.}
\end{marginfigure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Creating a PDF/A-2a compliant PDF document.},label={lis:Creating-a-PDF/A-2a},basicstyle={\footnotesize\ttfamily},breaklines=true]
public static void AttachSchema(Document doc, string profile, string resourcePath)
{
    XmpMetadata xmp = new XmpMetadata();
    PdfASchema pdfaSchema = null;
...
\begin{figure}
\includegraphics[scale=0.27]{figures/ch21/fig3}\caption{PDF/A-2a validation failed.}\label{fig:PDF/A-2a-validation-failed.}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Creating a PDF/A-3a compliant PDF document.},label={lis:Creating-a-PDF/A-3a},basicstyle={\footnotesize\ttfamily},breaklines=true]
public static void AttachSchema(Document doc, string profile, string resourcePath)
{
    XmpMetadata xmp = new XmpMetadata();
    PdfASchema pdfaSchema = null;
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding attachments to PDF/A-3a compliant PDF Document.},label={lis:Adding-attachments-to},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{figure}
\includegraphics[scale=0.32]{figures/ch22/fig1}\caption{Merging two PDF documents.}\label{fig:Merging-two-PDF}

\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Merging two documents.},label={lis:Merging-two-documents.},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Merging two documents.},label={lis:2Merging-two-documents.-1},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Appending a document.},label={lis:Appending-a-document.},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Appending two documents.},label={lis:Appending-two-documents.},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{marginfigure}
\includegraphics[scale=0.4]{figures/ch22/fig2}\caption{Extracting pages.}\label{fig:Extracting-pages.}
\end{marginfigure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Extracting pages.},label={lis:Extracting-pages.},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Merging three PDF documents.},label={lis:Merging-three-PDF},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
instances and a \texttt{Page}\index{Page} as the divider page.\begin{marginfigure}
\includegraphics[scale=0.26]{figures/ch22/fig7}\caption{Adding a page to a PDF.}\label{mar:Adding-a-page}
\end{marginfigure} After creating the divider page, you append the first document, insert
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding a page to a PDF.},label={lis:Adding-a-page},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
PDF document.
\begin{figure}
\includegraphics[scale=0.23]{figures/ch22/fig8}\caption{Adding page from existing PDF.}\label{fig:Adding-page-from}

\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding an existing page.},label={lis:Adding-an-existing},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{figure}
\includegraphics[scale=0.45]{figures/ch22/fig4}\caption{Splitting a PDF document.}\label{fig:Splitting-a-PDF}

\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Splitting a PDF.},label={lis:Splitting-a-PDF.},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF.Merger;
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Merging options.},label={lis:Merging-options.},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\includegraphics[scale=0.28]{figures/ch22/fig17}\caption{Using static merge options.}\label{fig:Using-static-merge}

\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Using static merge options.},label={lis:Using-static-merge},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Modifying dimensions of imported page.},label={lis:Modifying-dimensions-of},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.Merger;
using ceTe.DynamicPDF.PageElements;
...
\begin{figure}
\includegraphics[scale=0.30]{figures/ch22/fig12}\caption{Importing two pages.}\label{fig:Importing-two-pages.}

\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Importing two pages.},label={lis:Importing-two-pages.},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Importing page data.},label={lis:Importing-page-data.},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{figure}
\includegraphics[scale=0.34]{figures/ch22/fig14}\caption{Importing page data.}\label{fig:Importing-page-data.}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Clipping imported page.},label={lis:Clipping-imported-page.},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\protect\includegraphics[scale=0.35]{figures/ch22/fig15}\protect\caption{Clipping imported page.}\label{fig:Clipping-imported-page.}
...
\begin{marginfigure}
\includegraphics[scale=0.30]{figures/ch22/fig18}\caption{Reusing an ImportedPage.}\label{mar:Reusing-ImportedPageArea}
\end{marginfigure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Reusing ImportedPageArea.},label={lis:Reusing-ImportedPageArea.},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding content to an imported page.},label={lis:Adding-content-to},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\end{table}
You can then use the extracted element as desired.\marginnote{DynamicPDF Core Suite has no option to obtain a list of page elements
from an existing PDF.}  When working with
...
\begin{figure}
\includegraphics[scale=0.27]{figures/ch22/fig3}\caption{Adding a page.}\label{fig:Adding-a-page.}
\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding a Page.},label={lis:extract-Adding-a-Page.},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{figure}
\includegraphics[scale=0.45]{figures/ch22/fig6}\caption{Adding image to existing PDF document.}\label{mar:Adding-image-to}

\end{figure}
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Extracting image and adding to existing page.},label={lis:Extracting-image-and},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{figure}
\includegraphics[scale=0.48]{figures/ch22/fig9}\caption{Document to extract text from.}\label{fig:Document-to-extract}

\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Extracting document's text.},label={lis:Extracting-document's-text.},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{figure}
\includegraphics[scale=0.48]{figures/ch22/fig10}\caption{Extracted document text.}\label{fig:Extracted-document-text.}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Extracting text from area of a document.},label={lis:Extracting-text-from},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{figure}
\includegraphics[scale=0.48]{figures/ch22/fig11}\caption{Extracted document text.}\label{fig:Extracted-document-text.-1}
...
example provided by this chapter, there are alternative solutions.\marginnote{The \texttt{Outline} class has
no \texttt{Insert} method. This changes how you modify an existing outline
substantially.} 
...
\texttt{MergeDocument} instance's outline directly.
And, of course, as with the \texttt{PdfDocument} class, both the
...
\begin{marginfigure}
\includegraphics[scale=0.60]{figures/ch23/fig9}\caption{Merging two outlines.}\label{mar:Merging-two-outlines.}
\end{marginfigure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Merging two outlines.},label={lis:Merging-two-outlines.},basicstyle={\footnotesize\ttfamily},breaklines=true]
public static void SimpleMerge(string resourcePath, string outputPath)
{
...
\begin{marginfigure}
\includegraphics[scale=0.58]{figures/ch23/fig10}\caption{Consolidated outline.}\label{mar:Consolidated-outline.}
...
\begin{marginfigure}
\includegraphics[scale=0.45]{figures/ch23/fig1}\caption{Adding to an outline.}\label{mar:Adding-to-an}
\end{marginfigure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding to an outline.},label={lis:Adding-to-an},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\includegraphics[scale=0.58]{figures/ch23/fig12}\caption{Two PDF document outlines.}\label{mar:Two-PDF-document}
...
\begin{figure}
\includegraphics[scale=0.58]{figures/ch23/fig11}\caption{Manually merging two outlines.}\label{fig:Manually-merging-two}

\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Manually merging two outlines.},label={lis:Manually-merging-two},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Simple manually merging.},label={lis:Simple-manually-merging.},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{figure}
\includegraphics[scale=0.4]{figures/ch23/fig14}\caption{Doc1b.pdf contains a blank page with no bookmark.}\label{fig:Doc1b.pdf-contains-a}
\end{figure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Merging two outlines.},label={lis:Merging-two-outlines.-1},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Replacing an outline.},label={lis:Replacing-an-outline.},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
them using the following steps.\begin{marginfigure}
\includegraphics[scale=0.71]{figures/ch23/fig6}\caption{Root and link outline.}\label{mar:Root-and-link}
\end{marginfigure}
\begin{enumerate}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Changing outline captions and inserting.},label={lis:Changing-outline-captions},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{figure}
\includegraphics[scale=0.48]{figures/ch23/fig3}\caption{Changing outline captions and inserting outline.}\label{fig:Changing-outline-captions}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding bookmarks to existing PDF.},label={lis:Adding-bookmarks-to},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{figure}
\includegraphics[scale=0.32]{figures/ch23/fig4}\caption{Adding bookmarks to existing PDF.}\label{fig:adding-bookmarks-to}
...
\begin{tabular}{>{\raggedright}p{0.36\columnwidth}>{\raggedright}p{0.2\columnwidth}>{\raggedright}p{0.35\columnwidth}}
\toprule 
...
\begin{marginfigure}
\includegraphics[scale=0.30]{figures/ch24/fig14}\caption{Reading a form field's properties.}\label{mar:Reading-a-form}

\end{marginfigure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Reading a form field's properties.},label={lis:Reading-a-form},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
values.
\begin{marginfigure}
\includegraphics[scale=0.35]{figures/ch24/fig3}\caption{Reading form field values.}\label{fig:Reading-form-field}
\end{marginfigure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Reading form field values.},label={lis:Reading-form-field},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF.Merger;
using ceTe.DynamicPDF.PageElements;
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Reading two forms.},label={lis:Reading-two-forms.},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Completing a form.},label={lis:Completing-a-form.},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{figure}
\includegraphics[scale=0.58]{figures/ch24/fig1}\caption{Completing a form.}\label{fig:Completing-a-form.}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Completing multiple forms.},label={lis:Filling-Multiple-forms.},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{figure}
\includegraphics[scale=0.50]{figures/ch24/fig8}\caption{Completing multiple forms.}\label{fig:Completing-multiple-forms.}
\end{figure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Setting a form as readonly.},label={lis:readonly-form},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF.Cryptography;
using ceTe.DynamicPDF.Merger;
...
\begin{marginfigure}
\includegraphics[scale=0.35]{figures/ch24/fig5}\caption{Obtaining field locations.}\label{mar:Obtaining-field-locations.}
\end{marginfigure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Obtaining field locations.},label={lis:Obtaining-field-locations.},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF.Merger;
using ceTe.DynamicPDF.PageElements;
...
\begin{marginfigure}
\includegraphics[scale=0.34]{figures/ch24/fig2}\caption{Removing a form field.}\label{mar:Removing-a-form}
\end{marginfigure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Removing a form field.},label={lis:Removing-a-form},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Removing all form fields.},label={lis:Removing-all-form},basicstyle={\footnotesize\ttfamily},breaklines=true]]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Adding new fields.},label={lis:Adding-new-fields.},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{figure}
\includegraphics[scale=0.45]{figures/ch24/fig6}\caption{Adding new fields.}\label{fig:Adding-new-fields.}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Replacing form fields.},label={lis:Replacing-form-fields.},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\subsection{PositionPageElement and CreateLabel}\label{subsec:PositionPageElement-and-CreateLa}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Replacing form fields.},label={lis:Replacing-form-fields.-1},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{figure}
\includegraphics[scale=0.45]{figures/ch24/fig9}\caption{Merging two forms.}\label{fig:Merging-two-forms.}

\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Merging two forms.},label={lis:Merging-two-forms.},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Flattening a form.},label={lis:Flattening-and-removing},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{figure}
\includegraphics[scale=0.45]{figures/ch24/fig13}\caption{Flattening and removing individual form fields.}\label{fig:Flattening-and-removing-1}
\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Flattening and removing individual form fields.},label={lis:Flattening-and-removing-1},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={A very simple component.},label={lis:custom-label},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
...
\end{marginfigure}
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={A extended label.},label={lis:custom-label},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.PageElements;
using System;
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Using the read-only label.},label={lis:using-readonly-label},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\end{lstlisting}
\leavevmode\egroup
 Figure \ref{mar:extended-label} illustrates the component used in a Listing \ref{lis:using-textareal} where it adds the custom component to a PDF document's page.\begin{marginfigure}
\includegraphics[scale=0.5]{figures/ch21/fig8}\caption{An custom TextArea.}\label{mar:extended-label}
\end{marginfigure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Using custom TextArea.},label={lis:using-textareal},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={A custom star.},label={lis:custom-star},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.IO;
using ceTe.DynamicPDF.PageElements;
using System;
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Using the star.},label={lis:using-star},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Using chapter's custom components.},label={lis:using-custom-star},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
...
\leavevmode\egroup
\begin{figure}
\includegraphics[scale=0.33]{figures/ch21/fig11}\caption{Using custom components.}\label{fig:custom-comps}
...
Core Suite. The \texttt{ceTe.DynamicPDF\allowbreak.LayoutEngine}\index{ceTe.DynamicPDF.LayoutEngine}
...
\begin{figure}
\includegraphics[scale=0.22]{figures/ch27/fig28}\caption{DynamicPDF Designer.}\label{mar:DynamicPDF-Designer.}
...
\begin{figure}
\includegraphics[scale=0.20]{figures/ch27/fig23}\caption{Designer's development process.}\label{fig:Designer's-development-process.}
\end{figure}
...
\begin{figure}
\includegraphics[scale=0.20]{figures/ch27/fig24}\caption{Developer's development process.}\label{fig:Developer's-development-process.}
...
instance and another that takes the file path to a DLEX file for loading.\marginnote{You should almost never need to modify a DLEX file directly, instead use Designer.}
...
is a diagram of the XSD schema.
\begin{figure}
\includegraphics[scale=0.30]{figures/ch27/fig25}\caption{DLEX XML schema.}\label{fig:DLEX-XML-schema.}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Simple data object.},label={lis:Simple-data-object.},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Simple method to create a Book.},label={lis:Simple-method-to},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={A Book object serialized into JSON.},label={lis:A-Book-object},basicstyle={\footnotesize\ttfamily},breaklines=true]
{
...
\begin{marginfigure}
\includegraphics[scale=0.25]{figures/ch27/fig2}\caption{Data loaded in Designer.}\label{mar:Data-loaded-in}
\end{marginfigure}
...
\begin{figure}
\includegraphics[scale=0.5]{figures/ch27/fig5}\caption{RecordBox, Image, and RecordArea properties.}\label{mar:RecordBox-properties.}
\end{figure}

\begin{figure}
\includegraphics[scale=0.23]{figures/ch27/fig4}\caption{DLEX report created in Designer.}\label{fig:DLEX-report-created}
\end{figure}
...
\includegraphics[scale=0.30]{figures/ch27/fig32}\caption{The DLEX document for Book.}\label{fig:The-DLEX-document}
...
\begin{figure}[h]
\includegraphics[scale=0.20]{figures/ch27/fig10}\caption{Report output in Designer.}\label{fig:Report-output-in}
\end{figure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Generating report with simple data.},label={lis:Generating-report-with},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={A Chapter Class used with Book class.},label={lis:A-Chapter-Class},basicstyle={\footnotesize\ttfamily},breaklines=true]
namespace ChapterExamples.Chapters.ChapterTwentySeven.Data
{
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Modifying Book class to include a collection of Chapter class instances.},label={lis:Modifying-Book-class},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
namespace ChapterExamples.Chapters.ChapterTwentySeven.Data
{
    public class Book
    {
        ...
        public string ImagePath { get => imagePath; set => imagePath = value; }
        public string getJson() { return JsonSerializer.Serialize(this, new JsonSerializerOptions { WriteIndented = true });}
    }
}
\end{lstlisting}
\leavevmode\egroup
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Book JSON modified to include chapters.},label={lis:Book-JSON-modified},basicstyle={\footnotesize\ttfamily},breaklines=true]
{
  "Title": "DynamicPDF Core Suite By Example",
  "Author": "James A Brannan",
  "PublishDate": 2025,
  ...
  "Chapters": [
    ...
    {
      "Title": "Chapter 1",
      "Summary": "Summary of chapter 1\nOntinuedcay ethay imeTay."
    },
    {
      "Title": "Chapter 2",
      "Summary": "The summary of chapter 2\nOntinuedcay ethay imeTay avellerTra."
    },
--- snip ---
    {
      "Title": "Chapter 14",
      "Summary": "The summary of chapter 14\nOntinuedcay ethay imeTay avellerTray."
    }
  ],
  "ImagePath": "cover-image.png"
}
...
\begin{figure}
\includegraphics[scale=0.65]{figures/ch27/fig26}\caption{Assigning Chapters as the Report element's data name.}\label{mar:Assigning-Chapters-as}
\end{figure}
You then add the \texttt{Title} and \texttt{Summary} as \texttt{RecordBox} elements to the \texttt{Detail} element (Figure \ref{fig:Detail-section-in}).
\begin{figure}
\includegraphics[scale=0.26]{figures/ch27/fig12}\caption{Detail section in Designer.}\label{fig:Detail-section-in}
\end{figure}
...
\begin{figure}
\includegraphics[scale=0.30]{figures/ch27/fig31}\caption{DLEX data containing a Report and Detail.}\label{fig:DLEX-data-containing}
...
and \texttt{UseJson}, to demonstrate using an object model produces
the same results as JSON. 
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Creating a book report with chapters.},label={lis:Creating-a-book},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.LayoutEngine;
using ChapterExamples.Chapters.ChapterTwentySeven.Data;
using ChapterExamples.Utility;
...
\begin{figure}
\includegraphics[scale=0.26]{figures/ch27/fig13}\caption{The created report with chapters.}\label{fig:The-created-report}
...
\begin{figure}
\includegraphics[scale=0.4]{figures/ch27/fig33}\caption{Core Suite uses data names to reflect through data collections.}\label{fig:Core-Suite-uses}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Chapters implemented as an enumerable object.},label={lis:Chapters-implemented-as},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\begin{figure}
\includegraphics[scale=0.32]{figures/ch27/fig29}\caption{A simple DLEX report.}\label{fig:A-simple-DLEX}
\end{figure}
...
\begin{figure}
\includegraphics[scale=0.40]{figures/ch27/fig34}\caption{Data used for the enumerable report data.}\label{mar:Data-used-for}
...
\begin{marginfigure}
\includegraphics[scale=0.17]{figures/ch27/fig30}\caption{PDF report of chapters using enumerable data.}\label{mar:PDF-report-of}
\end{marginfigure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Providing enumerable report data.},label={lis:Providing-enumerable-report},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.LayoutEngine;
...
The \texttt{DataTableReportData}\index{DataTableReportData} class is part of the \texttt{ceTe.DynamicPDF\allowbreak.LayoutEngine.Data}\index{ceTe.DynamicPDF.LayoutEngine.Data}
...
\begin{figure}
\includegraphics[scale=0.35]{figures/ch27/fig18}\caption{Creating JSON in Azure Data Studio.}\label{fig:Creating-JSON-in}
...
\begin{figure}
\includegraphics[scale=0.18]{figures/ch27/fig16}\caption{Customers DLEX file in Designer.}\label{fig:Customers-DLEX-file}
...
\begin{figure}
\includegraphics[scale=0.3]{figures/ch27/fig27}\caption{Customers data set.}\label{fig:Customers-data-set.}
...
\begin{figure}
\includegraphics[scale=0.18]{figures/ch27/fig17}\caption{Finished customers report.}\label{mar:Finished-customers-report.}
\end{figure}

\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Report using database table.},label={lis:Report-using-database},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.LayoutEngine;
using ceTe.DynamicPDF.LayoutEngine.Data;
using System.Data;
...
class. The \texttt{DataReaderReportData}\index{DataReaderReportData} class is part of the \texttt{ceTe.DynamicPDF\allowbreak.LayoutEngine}\index{ceTe.DynamicPDF.LayoutEngine}
...
\begin{figure}
\includegraphics[scale=0.18]{figures/ch27/fig19}\caption{Orders DLEX report in Designer.}\label{fig:Orders-DLEX-report}
\end{figure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Report from database SQL statement.},label={lis:Report-from-database},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.LayoutEngine;
using ceTe.DynamicPDF.LayoutEngine.Data;
using System.Data.SqlClient;
...
\begin{figure}
\includegraphics[scale=0.20]{figures/ch27/fig22}\caption{Order report.}\label{fig:Order-report.}
\end{figure}
...
\begin{marginfigure}
\includegraphics[scale=0.20]{figures/ch28/fig10}\caption{Employees and territories DLEX.}\label{fig:Employees-and-territories-1}
\end{marginfigure}
...
\begin{figure}[h]
\includegraphics[scale=0.20]{figures/ch28/fig12}\caption{DLEX template data elements.}\label{fig:DLEX-template-data}
...
\begin{figure}
\includegraphics[scale=0.23]{figures/ch28/fig11}\caption{Employees and territories report.}\label{fig:Employees-and-territories-2}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Report with sub-report using JSON.},label={lis:Report-with-sub-report},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={EmployeeTerritory Object.},label={lis:EmployeeTerritory-Object.},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ChapterExamples.Utility;
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Report using business object.},label={lis:Report-using-business},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
After gathering the JSON data, the method deserializes it using \texttt{JsonConvert\allowbreak.DeserializeObject}\index{JsonConvert.DeserializeObject}. Then, you create
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Report using SQL and JSON.},label={lis:Report-using-SQL},basicstyle={\footnotesize\ttfamily},breaklines=true]
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Report with subreport.},label={lis:Report-with-subreport.},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.LayoutEngine;
using ceTe.DynamicPDF.LayoutEngine.Data;
using System.Data.SqlClient;
...
\begin{figure}
\includegraphics[scale=0.2]{figures/ch28/fig4}\caption{Simple place holder example.}\label{fig:Simple-place-holder}
\end{figure}
...
\begin{marginfigure}
\includegraphics[scale=0.34]{figures/ch28/fig15}\caption{Placeholder XSD element.}\label{mar:Placeholder-XSD-element.}
...
to be added.
\begin{marginfigure}
\includegraphics[scale=0.2]{figures/ch28/fig5}\caption{Cover page with placeholder.}\label{mar:Cover-page-with}
\end{marginfigure}
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Report with placeholder.},label={lis:Report-with-placeholder.},basicstyle={\footnotesize\ttfamily},breaklines=true]
using ceTe.DynamicPDF;
using ceTe.DynamicPDF.LayoutEngine;
...
\begin{figure}[h]
\includegraphics[scale=0.182]{figures/ch29/fig1}\caption{Dialog asking to create new or open existing DLEX.}\label{fig:Dialog-asking-to}
...
\begin{figure}[h]
\includegraphics[scale=0.32]{figures/ch29/fig2}\caption{Designer's toolbar.}\label{fig:Designer's-toolbar.}
\end{figure}
...
most elements and has properties
...
\begin{marginfigure}
\includegraphics[scale=0.27]{figures/ch29/fig38}\caption{Layout Data Editor dialog.}\label{mar:Layout-Data-Editor}
...
\begin{figure}
\includegraphics[scale=0.27]{figures/ch29/fig7}\caption{Creating a page in Designer.}\label{fig:Creating-a-page}
...
\begin{figure}
\includegraphics[scale=0.18]{figures/ch29/fig8}\caption{Previewing a report in Designer.}\label{fig:Previewing-a-report}
\end{figure}
\subsection{Report}
...
\begin{figure}[h]
\includegraphics[scale=0.18]{figures/ch29/fig9}\caption{Adding a Report in Designer.}\label{fig:Adding-a-Report}
\end{figure}
...
in the header.
\begin{marginfigure}
\includegraphics[scale=0.18]{figures/ch29/fig10}\caption{Adding a conditional footer.}\label{fig:Adding-a-conditional}
\end{marginfigure}
...
\begin{figure}[h]
\includegraphics[scale=0.18]{figures/ch29/fig11}\caption{Previewing report in Designer.}\label{fig:Previewing-report-in}
...
\begin{figure}
\includegraphics[scale=0.18]{figures/ch29/fig12}\caption{Report with Subreport in Designer.}\label{fig:Report-with-Subreport}
\end{figure}
...
\begin{figure}
\includegraphics[scale=0.18]{figures/ch29/fig14}\caption{Previewing report with subreport.}\label{fig:Previewing-report-with}
...
\begin{figure}
\includegraphics[scale=0.45]{figures/ch29/fig18}\caption{Designer Label element.}\label{fig:Designer-Label-element.}
\end{figure}
\noindent\begin{flushleft}
...
\begin{figure}
\includegraphics[scale=0.45]{figures/ch29/fig19}\caption{Designer Page Numbering Label element.}\label{fig:Designer-PageNumberingLabel-elem}
\end{figure}
...
\begin{figure}
\includegraphics[scale=0.5]{figures/ch29/fig17}\caption{Designer Formatted Record Area.}\label{fig:Designer-Formatted-Record}

\end{figure}
\begin{marginfigure}
\includegraphics[scale=0.24]{figures/ch29/fig20}\caption{Designer RecordArea element.}\label{fig:Designer-RecordArea-element.}
\end{marginfigure}
...
\begin{marginfigure}
\includegraphics[scale=0.4]{figures/ch29/fig16}\caption{Designer graphical elements.}\label{mar:Designer-graphical-elements.}
...
\begin{marginfigure}
\includegraphics[scale=0.15]{figures/ch29/fig25}\caption{Designer Soft Break element.}\label{fig:Designer-Soft-Break}
\end{marginfigure}
...
\begin{marginfigure}
\includegraphics[scale=0.35]{figures/ch29/fig15}\caption{Designer barcodes.}\label{mar:Designer-barcodes.}
...
to customize output. 
\begin{marginfigure}
\includegraphics[scale=0.2]{figures/ch29/fig41}\caption{Using an expression to determine order price.}\label{mar:Using-an-expression}
\end{marginfigure}
