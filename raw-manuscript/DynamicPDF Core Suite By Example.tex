...
\title[DynamicPDF Core Suite by Example]{\textcolor{white}{DynamicPDF Core Suite for .NET by Example}}
...
\SetWatermarkVerCenter{750pt}
\SetWatermarkText{\includegraphics[scale=0.20,angle=-45]{background.png}}
...
\noindent\begin{flushleft}
©2025. This work is openly licensed via CC BY 4.0. \includegraphics[scale=.5]{figures/cc-by.png}
\par\end{flushleft}
...
\color{black}
\BgThispage
Welcome to the DynamicPDF Core Suite, powerful software for creating
...
Part three completes our examination of the DynamicPDF Core Suite by examining creating custom components and dynamic reports using DynamicPDF's Layout Engine and its accompanying WSYWIG, DynamicPDF Designer. In this part, you learn how to create reports graphically using Designer and then programmatically use the Core Suite's Layout Engine to create dynamic reports. 
...
list of differences.\sidenote{A list of features and differences between Core Suite versions is
available at: (https://www.dynamicpdf.com/docs/dotnet/dynamic-pdf-core-suite-features} But in general, essentials is for creating very simple PDF documents
...
ISO standards for PDF documents, ISO 32000-1:2008 and ISO 32000-2:2020.\cite{iso_pdf1,iso_pdf2}
...
A PDF document can be linearized (optimized) or non-linearized.\sidenote{Linearize a PDF if creating a large document that requires streaming
on the web.} PDF documents can also conform to standards such as PDF/UA or PDF/A,
...
PDF documents are suitable for long-term archiving.\sidenote{We discuss PDF/UA and PDF/A more in Chapters \ref{chap:Accessibility}
and \ref{chap:Archiving-and-PDF/A}.}
...
\part{Creating PDF Documents}\label{part:Create-PDF-Documents}
\color{white}
\chapter{Documents}\label{chap:Creating-PDF-Documents}
\color{black}
\BgThispage
In this chapter, you learn about creating documents using the \texttt{Document} class. We build upon what you learned in Chapter \ref{chap:Introducing-DynamicPDF-Core}
...
document in a PDF reader such as Adobe Acrobat Reader. 
...
document to a file, to a stream, or a byte-array.
\begin{itemize}
...
PDF into a larger application such as a web page.\sidenote{In this book we limit our examples to using the \texttt{Draw} method
to output to a file.}
...
You add pages to a document by creating and adding \texttt{Page}\index{Page} instances.\marginnote{All measurements in Core Suite use points.}
...
classes. The \texttt{Page}\index{Page} class has eight different overloads for its constructor:
...
initialize a page's size, orientation, and layout.\marginnote{A page's defaults values are: 50 pt margins, 1008 pt width, and 612 pt
height.}
...
\ref{lis:ch2-multipage-Creating-a-PDF-1} prints the page's dimensions to the console as: \texttt{Width: 1008 Height: 612 LeftMargin: 100
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Ignoring an image's margins and relative positioning.},label={lis:Ignoring-an-image's},basicstyle={\small\ttfamily}]
...
left corner of the image. For example, the image used in Listing \ref{lis:Centering-an-image}
...
\begin{figure}
\includegraphics[scale=0.35]{figures/ch2/fig11}\caption{Centering an image.}\label{mar:centering-image}
\end{figure}
...
\begin{figure}
\includegraphics[scale=0.40]{figures/ch2/valign}\caption{Applying VAlign to a Label.}\label{mar:Applying-VAlign-to}
\end{figure}
...
\leavevmode\egroup
\begin{marginfigure}
\includegraphics[scale=0.41]{figures/ch2/custom-props}\caption{A custom property.}\label{mar:A-custom-property}
\end{marginfigure}
...
\color{black}
\BgThispage
In the previous chapter, you learned how to add pages to a document and how to precisely lay out a document's
...
\begin{marginfigure}
\includegraphics[scale=0.34]{figures/ch3/fig6}\caption{Applying a header and footer to a PDF document.}\label{fig:Applying-a-header}
\end{marginfigure}
...
\includegraphics[scale=0.46]{figures/ch3/fig5}\caption{Applying templates to a sectioned PDF document.}\label{fig:final-Example-of-applying}
...
\begin{marginfigure}
\includegraphics[scale=0.58]{figures/ch4/fig15}\caption{Visual Studio and conflicting namespace.}\label{mar:Visual-Studio-and}
...
\begin{marginfigure}
\raggedright
\includegraphics[scale=0.25]{figures/ch4/fig2}\caption{Adding two TextArea elements to a document.}\label{mar:Results-from-adding}
\end{marginfigure}
\bgroup\inputencoding{latin9}
...
In the previous chapter, you learned how to add textual elements to a PDF document. However, a document with only text is far less engaging than one that includes graphics and images. In this chapter, we enhance our PDFs by incorporating graphical elements. 
...
In the last chapter, you learned how to add vector-based primitive
...
This chapter teaches you how to treat page elements as a single unit.
...
In this chapter we discuss fonts and colors. Although we have addressed
...
Now that we have explored fonts and colors, let's return to page elements and learn how to add tables to a PDF document. In this chapter, you will learn how to create and work with tables, which present data in a structured format using rows and columns.
...
In this chapter and the next, you’ll explore the powerful charting capabilities of DynamicPDF Core Suite for .NET. With these tools, you can create detailed and visually striking charts that effectively convey complex information. By the end of the next chapter, you’ll likely agree that DynamicPDF Core Suite offers one of the best-in-class charting solutions for generating PDF documents. That said, grab a coffee—these next two chapters are among the most challenging in this book!
...
In the previous chapter, you learned how to create pie, bar, and column charts, as well as how to work with plot areas, legends, and multiple plot areas. This chapter builds on that foundation by introducing line, area, and scatter charts. Additionally, we explore how to customize the x and y axes, add symbols, tick marks, and grid lines, incorporate markers, and modify data labels.
...
A bar code represents data in a visual, machine-readable form. One-dimensional
...
A quick-response code (QR) is a two-dimensional matrix bar code that
contains black squares on a white background and is readable by imaging
devices like your cellphone. Listing \ref{lis:Adding-a-QR} illustrates adding a QR code to a PDF document. First, you create a
...
This chapter teaches you how to create bookmarks and add outlines
...
Two, linking to page ten. Finally, you loop through all child outline
...
When creating PDF documents, it's often crucial to ensure that only
...
certificates. \sidenote{There are numerous online resources providing instructions on creating
self-signed certificates. For example, https://learn.microsoft.com/en-us/entra/identity-platform/howto-create-self-signed-certificate} Although we do not discuss in detail how to create a self-signed
...
Event-driven programming (EDP) focuses on events an application listens
...
\begin{marginfigure}
\protect\includegraphics[scale=0.4]{figures/ch16/fig6}\protect\caption{Opening a file with \texttt{FileOpenAction}\index{FileOpenAction}.}\label{mar:Opening-a-file}
\end{marginfigure}
}
...
In this chapter, you learn how to add JavaScript to your PDF documents.
...
In the past several chapters, you learned about interactive forms,
...
In this chapter, you learn how to embed files as attachments in a
...
This chapter examines the PDF/UA (PDF/Universal Accessibility) standard
...
any resource type. Although first developed to define web content,
it is also widely used with PDF documents.

Core Suite provides the \texttt{XmpMetadata}\index{XmpMetadata}, \texttt{PdfUASchema}\index{PdfUASchema},
and \texttt{DublinCoreSchema}\index{DublinCoreSchema} classes for adding the required schema
to a PDF document. Listing \ref{lis:Adding-required-schemas} illustrates.
For brevity, you only add the two necessary properties to the Dublin
Core schema and exclude other descriptive properties available by
the schema.
...
\part{Modifying PDF Documents}\label{part:modifying-pdf-doc}

\color{white}
\chapter{Merging}\label{chap:Merging}
\color{black}
\BgThispage
In this chapter you learn how to merge PDF documents to create a new
...
public static void AddSingleImportedPage(string resPath, string outPath)
{
    ...
    mergedDoc.Draw(outPath + "merge-single-page-importedpage-out.pdf");
}
\end{lstlisting}
\leavevmode\egroup
...
In this chapter, you learn how to modify an existing PDF document's
...
In Chapter \ref{chap:Interactive-Forms}, you learned how to create forms. In subsequent chapters you also learned how to create forms that allow end users to pre-fill,
...
\color{black}
\BgThispage
In previous chapters, you created PDFs using code or by modifying existing
...
\bgroup\inputencoding{latin9}
\begin{lstlisting}[caption={Book JSON modified to include chapters.},label={lis:Book-JSON-modified},basicstyle={\scriptsize\ttfamily},breaklines=true]
...
In the last chapter you learned how to create reports using layout
...
\color{black}
\BgThispage
In this chapter, we examine the graphic elements available when using
